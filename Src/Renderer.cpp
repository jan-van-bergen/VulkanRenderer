...
void Renderer::create_gbuffer() {
	...
	VK_CHECK(vkCreateDescriptorSetLayout(device, &layout_create_info, nullptr, &gbuffer.descriptor_set_layouts.geometry));
	...
	VK_CHECK(vkCreateDescriptorSetLayout(device, &layout_create_info, nullptr, &gbuffer.descriptor_set_layouts.sky));
	...
	for (int i = 0; i < Texture::textures.size(); i++) {
		...
		alloc_info.pSetLayouts        = &gbuffer.descriptor_set_layouts.geometry;
	...
	pipeline_layout_details.descriptor_set_layouts = { gbuffer.descriptor_set_layouts.geometry };
	pipeline_layout_details.push_constants = { push_constants };

	gbuffer.pipeline_layouts.geometry = VulkanContext::create_pipeline_layout(pipeline_layout_details);

	pipeline_layout_details.descriptor_set_layouts = { gbuffer.descriptor_set_layouts.sky };
	pipeline_layout_details.push_constants = { };

	gbuffer.pipeline_layouts.sky = VulkanContext::create_pipeline_layout(pipeline_layout_details);
	...
	pipeline_details.pipeline_layout = gbuffer.pipeline_layouts.sky;

	gbuffer.pipelines.sky = VulkanContext::create_pipeline(pipeline_details);

	// Create Uniform Buffers
	gbuffer.uniform_buffers.resize(swapchain_views.size());
	...
	for (int i = 0; i < swapchain_views.size(); i++) {
		gbuffer.uniform_buffers[i] = VulkanMemory::buffer_create(aligned_size,
	...
	std::vector<VkDescriptorSetLayout> layouts(swapchain_views.size(), gbuffer.descriptor_set_layouts.sky);
	...
	for (int i = 0; i < gbuffer.descriptor_sets_sky.size(); i++) {
		...
		descriptor_ubo.buffer = gbuffer.uniform_buffers[i].buffer;
...
void Renderer::record_command_buffer(u32 image_index) {
	...
	vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gbuffer.pipelines.geometry);
	...
	for (int i = 0; i < scene.meshes.size(); i++) {
		...
		for (int j = 0; j < mesh.sub_meshes.size(); j++) {
			...
			if (first_sub_mesh) {		
				...
				vkCmdPushConstants(command_buffer, gbuffer.pipeline_layouts.geometry, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(GBufferPushConstants), &push_constants);
			}
			...
			if (last_texture_handle != sub_mesh.texture_handle) {
				...
				vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, gbuffer.pipeline_layouts.geometry, 0, 1, &texture.descriptor_set, 0, nullptr);
			}
...
void Renderer::swapchain_destroy() {
	...
	vkDestroyPipeline      (device, gbuffer.pipelines.geometry, nullptr);
	vkDestroyPipeline      (device, gbuffer.pipelines.sky,      nullptr);
	vkDestroyPipelineLayout(device, gbuffer.pipeline_layouts.geometry, nullptr);
	vkDestroyPipelineLayout(device, gbuffer.pipeline_layouts.sky,      nullptr);
